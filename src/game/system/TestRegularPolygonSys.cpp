#include "game/system/fwd.h"
#include "game/component/fwd.h"
#include "game/factories.h"
#include "box2d/box2d.h"
#include "game/system/TestRegularPolygon.h"

namespace
{
     BlockParams getSidesAndStyle()
     {
          BlockParams params;

          params.position.x = (std::rand() % 960) / 10 - 48.f;
          params.position.y = (std::rand() % 960) / 10 - 48.f;

          auto randNum = std::rand() % 100;

          // 40：40：15：5
          if (randNum < 40)
               params.polygon = {3, 3.f / 10};
          else if (randNum < 80)
               params.polygon = {4, 4.f / 10};
          else if (randNum < 95)
               params.polygon = {5, 5.f / 10};
          else
               params.polygon = {6, 6.f / 10};

          randNum = std::rand() % 100;
          if (randNum < 50)
               params.style = {BOLCK_FILL_GREEN, BOLCK_STROKE_GREEN};
          else if (randNum < 80)
               params.style = {BOLCK_FILL_BLUE, BOLCK_STROKE_BLUE};
          else if (randNum < 90)
               params.style = {BOLCK_FILL_PURPLE, BOLCK_STROKE_PURPLE};
          else
               params.style = {BOLCK_FILL_ORANGE, BOLCK_STROKE_ORANGE};

          return params;
     }
}

void TestRegularPolygonSys(ecs::EntityManager &em, b2WorldId &worldId, uint32_t &tick)
{
     auto view = em.getView<Input>();
     for (auto entity : view)
     {
          Input *input = em.getComponent<Input>(entity);
          if (input->state & 0b10)
          {
               // auto [sides, style] = getSidesAndStyle();
               // createEntityBlock(em, worldId, tick, {sides, sides / 10.f}, style, input->x, input->y);
          }
     }

     if (shapeEntityMap.size() <= 500)
     {
          createEntityBlock(em, worldId, tick, getSidesAndStyle());
     }
}